...
<?php
	include('includes/database.php');
							...
							if ($error < 0){
									...
									$update=mysqli_query($connection," INSERT INTO `post` (`user_id`,`post_image`,`content`,`created`)
...
<?php
	include("includes/database.php");
			...
			//$query=mysqli_query($connection,"SELECT * from `post` LEFT JOIN `a_user` on a_user.user_id = post.user_id order by post_id DESC") or die(mysqli_connect_errno());
			$query=mysqli_query($connection,"select * from a_user A join post P on P.user_id = A.user_id where P.user_id={$_SESSION['user_id']} group by P.created DESC ") or die(mysqli_connect_errno());
			//if($query){
...
?>
		...
		<center>
			<img src="<?php if(!($location=="upload/")){echo $location;} ?>">
		</center>
		<br>
		</div>
...
?>		
			<div class="comment-display"<?php echo $comment_id ?>>
				...
				&nbsp;&nbsp;&nbsp;<?php echo $row['name']; ?>
				<b class="time-comment"><?php echo $time = time_stamp($time); ?></b></div>
